...
% \usepackage{fourier}  % for brackets
...
% Isabelle
\newcommand{\spec}{{\mbox{spec}}}
\newcommand{\allI}{{\mbox{allI}}}
\newcommand{\exI}{{\mbox{exI}}}
\newcommand{\exE}{{\mbox{exE}}}
...
$\Spec = \{\mbox{``,",``(",``)",``.",}``\land",``\lor",``\longrightarrow", ``D"\}$
...
\begin{definition}
Substitution of the variable with term in formula:
$\varphi[b/x]$
...
\pagebreak
\begin{theorem}
(HERE $\vdash_w=\vdash$) Remember that $P(x)$ is just an apllication of the function P. It's not like the traditional notation $\varphi(x_1,\dots,x_n)$ that means that $\{x_1,\dots,x_n\}\subseteq \FV(\varphi)$.
...
\myprf
\\We have a hypothesis H:$(\forall x\in \Term. \WF(\varphi) \longrightarrow \Tau \vdash \varphi)$.
...
\begin{theorem}(ERRONEOUS VARIANT)\\
